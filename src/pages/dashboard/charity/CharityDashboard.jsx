...
const CharityDashboard = () => {
  ...
  const { data: pickups = [], isLoading: loadingPickups } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/donation-requests/pickups/${user.email}`
      )
      return res.data
    },
  })
  ...
  const { data: donations = [], isLoading: loadingDonations } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/donation-requests/received/${user.email}`
      )
      return res.data
    },
  })
  ...
  const pickupMutation = useMutation({
    mutationFn: async (id) =>
      await axiosSecure.patch(`/donation-requests/confirm-pickup/${id}`),
    onSuccess: () => {
  ...
  const reviewMutation = useMutation({
    mutationFn: async (data) =>
      await axiosSecure.post(`/donations/${data.donationId}/reviews`, data),
    onSuccess: () => {
  ...
  return (
    <div className="p-4 md:p-8 bg-gradient-to-br from-[#f3f4f6] to-[#e0f7fa] min-h-screen text-gray-800">
      ...
      <div className="bg-white shadow-xl rounded-lg p-4 md:p-6 mb-6 border-l-4 border-primary">
        ...
        <p>
          <span className="font-medium">Email:</span> {charity.email}
        </p>
        <p>
        ...
        <p>
          <span className="font-medium">Status:</span> {charity.status}
        </p>
      </div>
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Pickup Status: {pickup.pickupStatus}
                </p>
              </div>
              <button
      ...
      <div className="mt-8">
        ...
        <div className="grid gap-4">
          {donations.map((donation) => (
            ...
            >
              <p>
                <strong>{donation.donationTitle}</strong>
              </p>
        ...
        {selected && (
          ...
          >
            <h4 className="font-semibold mb-2">
              ✍️ Review for: {selected.donationTitle}
            </h4>
            <textarea
